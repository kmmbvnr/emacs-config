    ...
    for (i = 0; i < report.errors.length; i += 1) {
        ...
        if (entry) {
            if (isFinite(entry.line)) {
                print(filename + ':' + entry.line + ': ' + entry.reason);
            }
        }
    }
}
...
if (report.implieds) {
    for (i = 0; i < report.implieds.length; i += 1) {
        ...
        if (entry) {
            if (isFinite(entry.line)) {
                print(filename + ':' + entry.line + ': Implied global ' + entry.name);
            }
        }
    }
}
...
if (report.unused) {
    for (i = 0; i < report.unused.length; i += 1) {
        ...
        if (entry) {
            if (isFinite(entry.line)) {
                print(filename + ':' + entry.line + ': Unused variable ' + entry.name);
            }
        }
    }
}
